...
int main()
{
    ...
    FILE* logFile = fopen("matricula_quicksort.txt","w");
...
void quicksortRec(int esq, int dir) {
    ...
    while (i <= j) {
        while (strcmp(array[i].corDoCabelo, pivo.corDoCabelo) < 0 || equals(array[i].corDoCabelo, pivo.corDoCabelo) && strcmp(array[i].nome, pivo.nome) < 0){
            i++;
            comp++;  
        } 
        while (strcmp(array[j].corDoCabelo, pivo.corDoCabelo) > 0 || equals(array[j].corDoCabelo, pivo.corDoCabelo) && strcmp(array[j].nome, pivo.nome) > 0){
            j--;
            comp++;
        } 
        if (i <= j) {
